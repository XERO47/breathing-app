...
const ProgressPath = React.memo(({ progress, videoFrameBorderRadius, color }) => {
  ...
  useEffect(() => {
    ...
    const animate = (timestamp) => {
      setAnimatedProgress((prev) => {
        ...
        if (Math.abs(diff) < 0.05) return progress;
  ...
  }, [progress, animatedProgress]); // Added animatedProgress to dependencies

  const strokeColor = typeof color === 'function' ? color(theme) : (color || '#FFFFFF');
  const progressRatio = animatedProgress / 100;
  const svgPathLeft = `M ${width / 2} ${height} L ${cornerRadius} ${height} Q 0 ${height} 0 ${height - cornerRadius} V ${cornerRadius} Q 0 0 ${cornerRadius} 0 L ${width / 2} 0`;
  const svgPathRight = `M ${width / 2} ${height} L ${width - cornerRadius} ${height} Q ${width} ${height} ${width} ${height - cornerRadius} V ${cornerRadius} Q ${width} 0 ${width - cornerRadius} 0 L ${width / 2} 0`;
  ...
  return (
    ...
    >
      <path
        ...
        className="progress-path progress-path-left" strokeLinecap="round"
      />
      <path
        ...
        className="progress-path progress-path-right" strokeLinecap="round"
      />
    </svg>
  );
});
...
function App() {
  ...
  useEffect(() => {
    ...
    styleEl.innerHTML = `
      @keyframes progressBarPulse {0% { opacity: 0.8; } 50% { opacity: 0.4; } 100% { opacity: 0.8; }}
      @keyframes animatedBackground {0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; }}
      .progress-path {will-change: stroke-dashoffset; transform: translateZ(0);}
    `;
    document.head.appendChild(styleEl);
    return () => document.head.removeChild(styleEl);
  }, []);

  // Define the common footer component
  const AppFooter = () => (
    <Box
      component="footer"
      sx={{
        ...
        mt: 'auto', // Pushes footer to the bottom of a flex container
        textAlign: 'center',
      }}
    >
      <Typography
        variant="caption" // Using caption for smaller text
        sx={{ color: 'text.secondary' }} // Standard subtle text color
      >
        Made with ❤️ for CodeCircuit Hackathon
      </Typography>
    </Box>
  );
  ...
  if (!sessionStarted) {
    return (
      <>
        <AnimatedBackground />
        {/* Outer Box to manage flex layout for the entire view */}
        <Box
          sx={{
            display: 'flex',
          ...
          >
            ...
            <Button
              ...
              sx={(theme) => {
                ...
                return {
                  bgcolor: mainColor, color: 'white', padding: '12px 28px',
                  borderRadius: '25px', textTransform: 'none', fontSize: '1.1rem',
    ...
    <>
      ...
      <Box
        sx={{
          ...
          position: 'relative',
          zIndex: 1,
          // padding is applied here to give space around content AND footer
          padding: { xs: 2, sm: 3 }, 
        }}
      >
        {/* Content Area */}
        <Box
          sx={{
            ...
            flexDirection: 'column',
            alignItems: 'center',
            justifyContent: 'center', // Center content within this box
            textAlign: 'center',
            gap: { xs: '20px', sm: '24px' },
            width: '100%',
          }}
        >
          ...
          <Box sx={{ position: 'relative', display: 'inline-block' }}>
            <Box
              sx={{
                position: 'relative', width: 'clamp(280px, 80vw, 350px)',
                aspectRatio: '2 / 3', borderRadius: videoFrameBorderRadius,
                overflow: 'hidden', boxShadow: '0 8px 25px rgba(0,0,0,0.1)',
              }}
            >
              <Box
                sx={(theme) => ({
              ...
              >
                <BreathingAnimation
                  sessionHasBegun={sessionStarted} videoRef={videoRef}
                  initialIsMuted={isMuted} onActualMutedStateChange={handleActualMutedStateChange}
